...
const ExploreMenu = ({ category, setCategory }) => {
  return (
    <div className='flex flex-col gap-6 ' id='menu'>
      <h1 className='text-gray-600 font-bold ml-32 text-2xl'>Explore our menu</h1>
      <p className='md:max-w-[60%] max-w-[100%] text-lg ml-32'>Choose from a diverse menu featuning a delectable array of dishes. Our mission is to satisfy your cravings and elevate your dining experience, one delicious meal at a time.</p>
      <div className='flex justify-between items-center gap-8 text-center overflow-x-scroll my-5'>
        {menu_list.map((item, index) => {
          return (
            <div className='ml-10 mr-10' key={index} onClick={() => setCategory(prev => (prev === item.menu_name ? 'All' : item.menu_name))}>
